...
main = do
    ...
    let polymerString =  head $ map Text.unpack polymerText
        ...
        newPolymersList = alteredPolymers ['A' .. 'Z']  polymerString
    putStrLn $ show $ minimum $ map (length . completelyReact) $ newPolymersList  
...
-- given an uppercase Char, remove all instances of that same upper or lower char from a given string
removeChemicalPair :: Char -> [Char] -> [Char]
removeChemicalPair c polymer = filter (isNotChemical c) polymer
    where
        isNotChemical c x 
            | c == x         = False
            | c == toUpper x = False 
...
-- given a list of chemicals and a starting polymer, generate a list of new polymers generated by removing the chemicals 
-- from the starting polymer
alteredPolymers :: [Char] -> [Char] -> [[Char]]
alteredPolymers chemicals p = [removeChemicalPair c p | c <- chemicals]
